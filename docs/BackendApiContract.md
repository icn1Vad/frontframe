...
> 文档状态：V1 后端联调基线
>
> 最近核对：2026-07-17，已与当前应用端口、HTTP Adapter 和 OpenAPI 端点对照。
...
### 2.3 AI 状态：轮询（V1 接入目标）

V1 真实后端接入使用轮询，不使用 SSE 或 WebSocket。后端和最终接入后的前端
必须共同满足以下规则：
...
当前仓库的实际接入状态需要与上述目标区分：

- `src/infrastructure/http/polling.ts` 已提供取消、指数退避和五分钟超时能力；
- 业务页面目前仍绑定 Mock Adapter，尚未统一接入该轮询工具；
- 页面不可见时暂停，以及读取响应 `Retry-After` 调整间隔，尚未在 HTTP Client
  和页面层完成；
- 开始真实联调前，应先补齐上述两项能力，并为分类候选、通用审查和合同审查
  分别增加可取消的轮询接入测试。
