...
- `mock`：仅用于本地开发和未配置 WPS 时的降级验证，使用前端文本模拟原文定位
  和修订预览；生产后端不应把该模式作为正常响应。
- `wps`：服务端返回 WPS 会话后，前端动态加载 WebOffice SDK 并挂载真实 Word 编辑器。

## 推荐生产链路

1. 后端业务层根据租户、用户、任务和文件版本生成短时编辑器会话。
2. HTTP Adapter 请求 `GET /api/v1/contract-review/tasks/{taskId}/editor-session`。
3. 当前 Next API `/api/contract-review/tasks/{taskId}/editor-session` 仅作为本地兼容代理；真实后端按 OpenAPI V1 路径实现。
...
本地联调可以复制 `.env.example` 为 `.env.local`，配置 WPS SDK、AppID、联调文件 ID 和短期 Token。正式环境应使用后端会话接口，不应在前端环境变量中存放长期凭证。
