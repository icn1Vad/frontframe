...
文件分类首次进入时保留大上传入口；点击选择或拖入文件后直接提交并进入“分类确认
列表”，不再经过独立的整页待上传阶段。进入分类确认后，“继续上传”在浮窗中完成
待上传清单和确认；确认上传后浮窗关闭，新文件直接出现在当前分类列表。人工确认成功
后保留当前列表，并在反馈区提供“查看分类任务池”入口。
...
- 合同任务不重复进入通用审查任务池。
- 合同上传配置页的“开始审查”会创建并启动任务，然后直接进入合同审查工作台。
...
分类任务池执行“发起审查”成功后直接进入审查任务池，减少完成状态变更后的再次寻路。
